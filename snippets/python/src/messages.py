...
def sign_message(sdk_services):
    ...
    try:
        sign_message_request = breez_sdk.SignMessageRequest(message=message)
        sign_message_response = sdk_services.sign_message(req=sign_message_request)
...
def check_message(sdk_services):
    ...
    try:
        check_message_request = breez_sdk.CheckMessageRequest(
            message=message,
            pubkey=pubkey,
            signature=signature)
        check_message_response = sdk_services.check_message(req=check_message_request)
