...
def prepare_redeem_onchain_funds(sdk_services, fee_rate):
    try:
        ...
        req = breez_sdk.PrepareRedeemOnchainFundsRequest(
            to_address=destination_address,
            sat_per_vbyte=sat_per_vbyte)
        result =  sdk_services.prepare_redeem_onchain_funds(req=req)
...
def redeem_onchain_funds(sdk_services, to_address, fee_rate):
    try:
        ...
        req = breez_sdk.RedeemOnchainFundsRequest(
            to_address=to_address,
            sat_per_vbyte=fee_rate)
        result = sdk_services.redeem_onchain_funds(req=req)
